...
### 기본 서버 설정 및 상태 확인
- [ ] `GET /` 요청 시 200 OK와 함께 서버 상태 메시지를 반환해야 한다.

### 이미지 분석 API (`POST /analyze`)
- [ ] 이미지 파일 없이 요청 시 400 Bad Request 에러를 반환해야 한다.
- [ ] 유효한 이미지 파일 업로드 시 200 OK와 분석 결과 JSON을 반환해야 한다.
- [ ] 업로드된 이미지가 `shared_volume/uploads` 폴더에 실제로 저장되어야 한다.
- [ ] 분석 결과가 `shared_volume/results` 폴더에 JSON 파일로 저장되어야 한다.
