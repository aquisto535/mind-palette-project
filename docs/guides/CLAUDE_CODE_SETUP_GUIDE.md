...
## 9. Claude 4.6 모델 스위칭 & Rate Limit 방어 전략

Claude Code Pro 사용 시 **토큰/메시지 한도 초과를 방지**하기 위한 핵심 워크플로우입니다.

### 🔄 상황별 모델 선택 가이드

| 모델 (기어) | 용도 및 프롬프트 예시 | 특징 |
|------------|-----------------------|------|
| **Haiku 4.6** | "이 파일 PEP8 포맷팅 해줘", "import 경로 일괄 수정해" | 로켓처럼 빠르고 싸다. 단순 타이핑 및 보일러플레이트 작성용. |
| **Sonnet 4.6** | "`tdd_plan.md`의 Cycle 1을 진행해줘", "이 에러 로그를 보고 문법 에러 고쳐줘" | **기본 모델**. 속도와 지능의 황금 밸런스. 단위 TDD 루프에 최적. |
| **Sonnet 4.6 (Thinking)** | "PyTorch vs ONNX L2 Norm 차이가 1e-1인 이유를 찾아줘" | 복잡도 높은 디버깅, 미묘한 버그 추적용. 소모량 높음. |
| **Opus 4.6** | (사용 자제) 아키텍처 붕괴 시, 대규모 구조 리팩토링 | 최고 지능이지만 **매우 비싸고 느림**. 이 체급은 Antigravity에게 위임 권장. |

### 🛡️ Rate Limit 방어 3원칙

1. **`/clear` 의 마법 (가장 중요)**
   - TDD 1개 Cycle 완료 및 `git commit` 후에는 **무조건 `/clear` 명령어로 대화 기록을 초기화**하세요.
   - 컨텍스트 창이 비워져 토큰 소모가 1/10로 줄어듭니다. 이전 맥락은 커밋된 코드와 `plan.md`를 통해 충분히 유지됩니다.
2. **작업 범위 강제 제한 (Micro-tasking)**
   - "Step 2 다 해줘" (X) → "오늘은 `test_onnx_conversion.py` 파일 하나만 작성해" (O)
   - 봇이 불필요한 파일을 스캔하며 토큰을 낭비하는 것을 막습니다.
3. **로그 던지기 자제**
   - 터미널 에러 로그를 통째로 복사해 붙여넣지 마세요.
   - `"pytest -v 실행해보고, 에러 원인 파악해서 고쳐"` 라고 지시하면 Claude Code가 직접 에러의 핵심만 읽어 들입니다.
