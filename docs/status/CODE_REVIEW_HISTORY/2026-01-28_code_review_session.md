...
### 10. OpenCV 파라미터 선택 이유 (Context7 분석 + 실측 벤치마크)

**질문**: 
- 왜 512×512로 리사이즈?
- 왜 GaussianBlur는 5×5, MedianBlur는 3×3?

> 🧪 **실측 벤치마크 추가 (2026-02-19)**  
> 테스트 환경: Windows x64, MSVC Release(`/O2 /MD`), OpenCV 4.11  
> 대상 이미지: `IMG_1294.jpg` (800×1131, 약 0.9MP)  
> 측정 방법: 각 연산을 30회 반복 후 **중앙값(Median)** 채택
...
**🧪 실측: GaussianBlur 커널 크기별 처리 시간 (512×512 입력)**

| 커널 크기 | 실측 시간 | 상대 비용 | 특징 |
|-----------|-----------|-----------|------|
| 3×3 | **0.285 ms** | 1.14x | 블러 약함, 노이즈 제거 미흡 |
| **5×5** | **0.249 ms** | **1.00x** ✅ | 최적 균형 (가장 빠르고 효과적) |
| 7×7 | **0.483 ms** | 1.94x | 약 2배 느림, 에지 뭉개짐 |

> **발견**: 5×5가 3×3보다 오히려 빠른 것은 SIMD/AVX 최적화 효과 (5×5는 분리 가능한 커널로 더 효율적으로 최적화됨). 7×7은 약 2배 느리므로 성능 패널티가 명확함.

**OpenCV 공식 권장**: `GaussianBlur(img, (5,5), 0)` 표준
...
**🧪 실측: MedianBlur 커널 크기별 처리 시간 (512×512 입력)**
...
> **발견**: 5×5 MedianBlur는 3×3보다 **4.5배 느림** (정렬 복잡도 O(n log n) 때문). GaussianBlur 5×5가 0.249ms인 것에 비하면, MedianBlur 5×5 (2.657ms)는 **약 10.7배 더 느림**. 3×3 선택은 단순한 선호가 아닌 성능상 명확한 근거가 있음.

**왜 GaussianBlur보다 작은 커널?**:
1. **연산 복잡도**: GaussianBlur O(n) vs MedianBlur O(n log n) (정렬 필요)
2. **에지 보존**: 비선형 필터로 에지 보존 우수, 3×3이 최소 유효 크기

---

#### **10.4 파이프라인 순서 (Resize 선행의 실측 효과)**

**🧪 실측: Resize 전후 GaussianBlur 처리 시간 비교 (원본 800×1131)**

| 전략 | 실측 시간 | 픽셀 수 비율 |
|------|-----------|-------------|
| 원본(800×1131)에 GaussianBlur 먼저 | **0.954 ms** | 3.45x |
| **Resize(512×512) 후 GaussianBlur** | **0.192 ms** | 1.0x |
| **실측 속도 향상** | **4.97배 빠름** ✅ | |

> **발견**: 픽셀 수 비율(3.45x)보다 실제 속도 향상(4.97x)이 더 큰 것은, 512×512가 CPU L2 캐시에 잘 맞아 캐시 히트율이 높아지기 때문. 이론보다 더 큰 실익이 있음.

**🧪 실측: 전체 파이프라인 처리 시간**

```
Resize(512×512) → GaussianBlur(5×5) → MedianBlur(3×3) → cvtColor(Grayscale)
총 처리 시간: 2.31 ms  (원본 이미지 800×1131 기준)
```

> 목표 성능 기준(`< 100ms`)에 비해 약 **43배의 여유**가 있음.

**파이프라인 순서**:
```cpp
// Step 1: Resize (512×512) → 이후 모든 연산의 픽셀 수 3.45배 감소
cv::resize(processed, processed, cv::Size(512, 512));

// Step 2: Gaussian (5×5) → 0.249ms (가장 빠름)
cv::GaussianBlur(processed, processed, cv::Size(5, 5), 0);

// Step 3: Median (3×3) → 0.590ms (5×5 대비 4.5배 절약)
...
**벤치마크 소스**: `preprocess-server/benchmark/opencv_benchmark.cpp`  
**실행 일시**: 2026-02-19
