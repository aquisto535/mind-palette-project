...
## 7. EC2 배포 환경 E2E 실측 (c5.large)

> 환경: AWS EC2 c5.large (2 vCPU, 4GB RAM), ONNX Runtime CPU 추론, 실제 아동 인물화 이미지
> 측정: 연속 요청 기준, Server-Timing 헤더 미적용 구간의 전체 E2E 응답시간

| 요청 순서 | E2E 응답시간 | 비고 |
| -------- | ----------- | ---- |
| 1번 (첫 요청) | ~280ms 후반 | 프로세스 워밍업 (ONNX 세션 초기화, OS 페이지 매핑) |
| 이후 평균 | **~170ms** | 안정 구간 (ONNX 세션 캐시 적중, 커널 캐시 활성화) |

**의미**:

- 첫 요청은 ONNX Runtime InferenceSession 초기화 + Linux 페이지 캐시 미스가 겹쳐 ~280ms 후반 소요
- 2번째 요청부터는 세션이 이미 생성되어 있고 이미지 처리 커널이 워밍업된 상태로 평균 **~170ms** 수렴
- 로컬 TensorRT 환경(14ms AI 추론)과 달리 EC2 CPU 환경에서는 C++ 전처리(~97ms) + ONNX 추론(~20ms) + Gateway 오버헤드의 합산이 지배적
- HFD 검사 특성상 연속 요청 패턴이므로 **안정 구간 170ms가 실사용 체감 지표**

---

## 8. EC2 인스턴스 비용 비교 (ADR-027)
