...
#### Problem Statement — 왜 단순 그레이스케일이 부적합한가?

가장 단순한 후보는 **1채널 그레이스케일을 3번 복제**(`cvtColor GRAY2BGR`)해서 EfficientNet-B2의 3채널 입력 슬롯을 채우는 것이지만, 아동 인물화(HFD)는 ImageNet 사진과 본질적으로 다른 도메인이라 다음 세 가지 문제가 발생합니다:

1. **정보의 휘발성 (Information Sparsity)**
   - HFD 스케치는 95% 이상이 여백(흰색)이며, 정보는 얇은 선(검정색)에 집중되어 있음
   - 그레이스케일 단일 채널만으로는 "95% 흰색 + 5% 검은 선"이라는 매우 희소한 신호만 전달됨
   - ImageNet 사전학습 모델은 자연 사진의 풍부한 픽셀 분포를 가정하므로, 이 희소성을 노이즈로 오인할 가능성이 큼

2. **다층적 정보의 압축 손실**
   - HFD에서 의미 있는 정보는 **세 가지 차원**으로 공존
     - 선의 굵기 (필압) → 심리적 상태 신호
     - 선의 연결성 (형태) → 윤곽 구조
     - 선의 골격 (구조) → 끊어진 선의 추론 가능성
   - 단일 그레이스케일 채널에는 이 세 차원이 섞여 있어 모델이 분리해 학습하기 어려움
   - 3번 복제해도 동일 정보의 중복일 뿐, **새로운 신호가 추가되지 않음**

3. **전이 학습 도메인 갭 (Domain Adaptation Gap)**
   - Pretrained EfficientNet-B2는 3채널 RGB 입력을 기대하지만, 통계적으로도 ImageNet과 HFD는 분포가 매우 다름
   - 단순 복제는 채널 수만 맞출 뿐, **통계적 분포 차이**는 그대로 둠 → 사전학습 가중치 활용 효율이 떨어짐

#### 정량적 도메인 갭 분석 (Synthetic Sketch Dataset N=20)

| Metric | ImageNet (Standard) | HFD Sketch (Target) | Gap |
|:---|:---:|:---:|:---|
| **Mean (Channel 0)** | 0.485 | **0.972** | +100.4% (배경이 압도적으로 밝음) |
| **Mean (Channel 1)** | 0.456 | **0.031** | **-93.2%** (선 정보가 매우 희소) |
| **Std (Channel 0)** | 0.229 | 0.156 | -31.8% (픽셀 편차가 낮음) |

→ ImageNet 기본 정규화 파라미터를 그대로 사용하면 모델은 입력을 비정상 노이즈로 해석할 가능성이 큼.
실제 데이터 통계(`mean=0.972, 0.031`)를 적용하고, **채널마다 보완적 정보**를 담아 도메인 갭을 좁혀야 함.
...
#### 0️⃣ 단순 그레이스케일 vs Hybrid 3채널 직접 비교

| 측면 | 그레이스케일 1채널 (3복제) | Hybrid 3채널 (채택) |
|:---|:---|:---|
| 채널 수 호환 | ✅ 가능 | ✅ 가능 |
| 정보 밀도 | ❌ 동일 정보 × 3, 중복 | ✅ 필압 + 형태 + 골격 (3차원) |
| 굵기 정보 | ❌ 알 수 없음 | ✅ B 채널의 distance transform이 명시 |
| 끊어진 선 추론 | ❌ 끊어진 그대로 | ✅ B 채널의 거리 정보가 구조적 연결성 보존 |
| ImageNet 도메인 호환 | ⚠️ 채널만 호환, 통계 분포 차이 큼 | ✅ White Background + 채널별 의미로 갭 축소 |
| 모델 Attention 분리 | ❌ 단일 신호만 | ✅ 채널별로 다른 시각적 특징 학습 가능 |
...
- **B (Dist)**: `DistanceTransform`을 통해 선의 중심부일수록 명암을 높여, 선의 굵기와 골격(Skeleton) 정보를 수치화합니다. **선이 얇거나 일부 소실된 경우에도 구조적 힌트를 제공**해 모델 강건성 향상.
...
- **Custom Normalization**: ImageNet 기본 `mean/std` 대신 실제 스케치 데이터셋에서 추출한 통계값을 사용해 도메인 최적화 (Python AI 서버에서 수행).
...
- ✅ **장점**: 단순 이진화 대비 AI 모델의 분류 정확도 향상, C++ 측의 Float 단위 정량 수치 추출 및 API 스키마 통합 가능.
- ⚠️ **주의**: Python 서버에서 입력 정규화 시 일반적인 ImageNet Mean/Std 대신 실제 전처리 결과물의 통계값 적용 필요. 정량 수치 정보는 API Gateway를 거쳐 최종 UI(XAI 관점)까지 무손실 전파되어야 함.
...
Claude Code는 대화가 길어질수록 컨텍스트 창(Context Window)에 이전 메시지가 누적됩니다. 이 누적이 한도에 가까워지면 시스템이 조용히 자동 압축을 수행하는데, 이 과정에서 **개발자가 인지하지 못한 채 응답 품질이 저하**되는 문제가 발생할 수 있습니다.

협업 도구인 **Antigravity**는 이미 자체적인 컨텍스트 관리 메커니즘을 내장하고 있었습니다. 작업이 일정 수준 누적되면 아래와 같은 배너를 출력하고 새 대화 시작을 명시적으로 요구합니다.

```text
● [System Auto-Check] 작업이 완료/저장되었습니다.
  현재 작업의 컨텍스트를 압축 보관했습니다. 토큰 오염(Context Rot)을
  방지하기 위해, 여기서 대화를 멈추고 IDE에서 [New Conversation]을 열어
  다음 작업을 지시해 주세요.
```

반면 Claude Code에는 동등한 기능이 부재했습니다. 이로 인해 두 도구 간 **컨텍스트 관리 일관성**이 결여된 상태였습니다.

### 의사결정 (Decision)

**Claude Code의 `Stop` 훅(Hook)**을 활용하여 응답 횟수를 자동으로 카운팅하고, 임계값 초과 시 경고 배너를 출력하는 시스템을 구축합니다.

#### 구성 요소

1. **카운터 스크립트** (`~/.claude/scripts/context-monitor.js`):
   - 각 `Stop` 이벤트(Claude 응답 완료)마다 실행
   - `~/.claude/context-counter.json`에 세션별 응답 횟수 기록
   - 세션 구분: `CLAUDE_SESSION_ID` 환경변수 우선, 없으면 1시간 비활성 기준으로 새 세션 판별
   - 임계값(`THRESHOLD = 20`) 도달 시 경고 배너 출력 후 카운터 리셋
...
2. **훅 등록** (`~/.claude/settings.json`):
   ...
   {
     "hooks": {
       "Stop": [{ "hooks": [{ "type": "command", "command": "node \"~/.claude/scripts/context-monitor.js\"" }] }]
     }
   }
   ```
...
```text
╔════════════════════════════════════════════════════════╗
║  ⚠️  [System Auto-Check] 컨텍스트 누적 경고           ║
╠════════════════════════════════════════════════════════╣
║  이 대화에서 20  번 응답했습니다.                      ║
║  권장 조치: /compact | 새 대화 | /clear                ║
╚════════════════════════════════════════════════════════╝
```

### 근거 (Rationale)

#### 1️⃣ Claude Code 네이티브 메커니즘 활용

- `Stop` 훅은 Claude Code가 공식 지원하는 이벤트 훅으로, 외부 의존성 없이 순수 Node.js(런타임 내장)로 동작합니다.
- 별도의 백그라운드 프로세스나 데몬이 필요 없어 시스템 리소스 부담이 없습니다.

#### 2️⃣ 비침습적(Non-Intrusive) 설계

- 정상 응답 흐름을 차단하지 않습니다(카운터 저장 실패 시 예외 무시, 훅 자체 오류가 대화를 중단시키지 않음).
- 임계값 이전에는 출력이 전혀 없으므로 **소음(Noise) 없이 임계점에서만 개입**합니다.
...
- **`/compact` 수동 실행**: 사용자가 직접 판단해야 하므로 누락 가능성 있음. ❌ Rejected (자동화 불가)
- **CLAUDE.md 규칙 추가**: Claude가 자체 판단으로 경고하는 방식. 신뢰성이 낮고 응답마다 소비되는 토큰이 증가. ❌ Rejected
- **`/loop` 스킬 주기적 실행**: 수동 트리거가 필요하고 시간 기반이라 실제 응답 횟수와 괴리 발생. ❌ Rejected

### 결론 (Consequences)

- ✅ **자동화**: 개발자가 별도로 신경 쓰지 않아도 20회 응답 후 자동으로 컨텍스트 상태를 알림.
- ✅ **일관성**: Antigravity와 동등한 수준의 컨텍스트 안전망을 Claude Code에도 확보.
