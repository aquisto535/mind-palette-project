...
- **C++ 터미널 테스트 실행 (DLL Load Error)**: `unit_tests.exe` 등 C++ 바이너리는 터미널(PowerShell)에서 직접 실행 시 `vcpkg` 런타임 DLL 경로를 찾지 못해 무반응 종료(Exit Code 1)될 수 있습니다. 런타임 검증은 반드시 **Visual Studio의 '테스트 탐색기'**를 활용하거나, 전용 개발자 쉘(VsDevCmd) 환경에서 수행하세요.
